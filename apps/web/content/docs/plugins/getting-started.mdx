...
Copy the output to your Dripnex plugins directory:

<Tabs items={['macOS', 'Windows', 'Linux']}>
  <Tab value="macOS">
    ```bash
    cp -r dist/ ~/Library/Application\ Support/Dripnex/plugins/my-plugin/
    ```
  </Tab>
  <Tab value="Windows">
    ```bash
    xcopy /E dist\ %APPDATA%\Dripnex\plugins\my-plugin\
    ```
  </Tab>
  <Tab value="Linux">
    ```bash
    cp -r dist/ ~/.config/Dripnex/plugins/my-plugin/
    ```
  </Tab>
</Tabs>
