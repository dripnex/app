...
import type { AiCommandDefinition, AiCommandPreset } from './ai-command-types.js';
...
describe('validateAiCommandDefinition', () => {
  ...
  it('requires id', () => {
    ...
    expect(errors.some((e: { field: string; message: string }) => e.field === 'id')).toBe(true);
  });
  ...
  it('requires name', () => {
    ...
    expect(errors.some((e: { field: string; message: string }) => e.field === 'name')).toBe(true);
  });

  it('requires systemPrompt', () => {
    const errors = validateAiCommandDefinition({ ...validCommand, systemPrompt: '' });
    expect(errors.some((e: { field: string; message: string }) => e.field === 'systemPrompt')).toBe(
      true
    );
  });
  ...
  it('requires userPromptTemplate', () => {
    ...
    expect(
      errors.some((e: { field: string; message: string }) => e.field === 'userPromptTemplate')
    ).toBe(true);
  });

  it('validates outputTarget enum', () => {
    const errors = validateAiCommandDefinition({ ...validCommand, outputTarget: 'invalid' });
    expect(errors.some((e: { field: string; message: string }) => e.field === 'outputTarget')).toBe(
      true
    );
  });
...
describe('validateAiCommandPreset', () => {
  ...
  it('requires version', () => {
    ...
    expect(errors.some((e: { field: string; message: string }) => e.field === 'version')).toBe(
      true
    );
  });
  ...
  it('requires commands array', () => {
    ...
    expect(errors.some((e: { field: string; message: string }) => e.field === 'commands')).toBe(
      true
    );
  });
  ...
  it('rejects empty commands array', () => {
    ...
    expect(
      errors.some((e: { field: string; message: string }) =>
  ...
  it('detects duplicate command ids', () => {
    ...
    expect(
      errors.some((e: { field: string; message: string }) => e.message.includes('Duplicate'))
    ).toBe(true);
  });
